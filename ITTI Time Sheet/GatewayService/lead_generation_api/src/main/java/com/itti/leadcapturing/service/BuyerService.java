...
// //     // File: src/main/java/com/itti/leadcapturing/service/BuyerService.java
// // // ✅ ADD THIS NEW METHOD (keep existing methods)
...
// //     try {
// //         log.info("📥 Fetching buyers for Org Admin ID: {}", adminId);
...
// //     } catch (Exception e) {
// //         log.error("❌ Error fetching buyers for admin", e);
// //         throw new RuntimeException("Failed to fetch buyers: " + e.getMessage(), e);
// //     }
// // }

// //     /**
// //      * ✅ FIXED: Create buyer with complete nested hierarchy
// //      */
// //     @Transactional
...
// //             // ✅ CRITICAL: Process each location and set bidirectional relationships
...
// //                 // ✅ Set buyer reference on location
// //                 location.setBuyer(buyer);
// //                 location.setIsDeleted(false);
                
// //                 // ✅ Initialize departments if null
...
// //                     // ✅ Set location reference on department
...
// //                         // ✅ CRITICAL: Set ALL parent references on user
...
// //             // ✅ Save buyer (cascade will save locations, departments, and users)
// //             log.info("");
...
// //             int totalLocs = savedBuyer.getLocations() != null ? savedBuyer.getLocations().size() : 0;
// //             int totalDepts = 0;
// //             int totalUsers = 0;
            
// //             log.info("📊 SAVED TO DATABASE:");
// //             log.info("   Locations: {}", totalLocs);
            
// //             if (savedBuyer.getLocations() != null) {
// //                 for (Location loc : savedBuyer.getLocations()) {
// //                     log.info("      ✅ Location '{}' (ID: {})", loc.getLocationName(), loc.getId());
                    
// //                     if (loc.getDepartments() != null) {
// //                         totalDepts += loc.getDepartments().size();
                        
// //                         for (Department dept : loc.getDepartments()) {
// //                             log.info("         ✅ Department '{}' (ID: {})", dept.getDepartmentName(), dept.getId());
...
// //                                 }
// //                             } else {
// //                                 log.error("            ❌ Department has NULL users list!");
// //                             }
// //                         }
// //                     } else {
// //                         log.error("         ❌ Location has NULL departments list!");
// //                     }
// //                 }
// //             }
            
// //             log.info("   Total Departments: {}", totalDepts);
// //             log.info("   Total Users: {}", totalUsers);
            
// //             if (totalDepts == 0) {
// //                 log.error("");
// //                 log.error("❌❌❌ NO DEPARTMENTS WERE SAVED!");
// //             }
            
// //             if (totalUsers == 0) {
// //                 log.error("");
// //                 log.error("❌❌❌ NO USERS WERE SAVED!");
// //             }
...
// //         } catch (Exception e) {
// //             log.error("❌ Error fetching buyer with ID: {}", id, e);
...
// //             // ✅ Update locations, departments, and users
...
// //                                     // Only encode password if provided (for new users or password changes)
...
//     // ============================================
//     // 🆕 ORGANIZATION ADMIN CREATES BUYER
//     // ============================================
    
//     /**
//      * ✅ NEW: Organization Admin creates a buyer with complete hierarchy
//      * This method handles the complete nested structure sent from frontend
//      */
//     @Transactional
//     public Buyer createBuyerByOrganizationAdmin(BuyerCreateByAdminDTO dto, Long orgAdminId) {
//         try {
...
//     /**
//      * ✅ NEW: Get buyers created by Organization Admin
//      */
// /**
//  * ✅ FIXED: Get buyers created by Organization Admin with proper lazy loading handling
//  */
// @Transactional(readOnly = true)
// public List<Buyer> getBuyersByOrganizationAdmin(Long adminId) {
//     try {
...
//         // ✅ CRITICAL: Initialize all lazy collections within transaction
...
//                 // Initialize departments in each location
//                 for (Location location : buyer.getLocations()) {
//                     if (location.getDepartments() != null) {
...
//                         // Initialize users in each department
//                         for (Department department : location.getDepartments()) {
//                             if (department.getUsers() != null) {
...
// }
//     /**
//      * ✅ NEW: Get buyers by organization company name
//      */
//     @Transactional(readOnly = true)
...
//     /**
//      * ✅ EXISTING: Create buyer with complete nested hierarchy
//      */
//     @Transactional
//     public Buyer createBuyer(Buyer buyer) {
//         try {
...
//    @Transactional(readOnly = true)
//     public Buyer getBuyerByIdForOrgAdmin(Long buyerId, Long orgAdminId) {
//         try {
//             log.info("========================================");
//             log.info("📥 GET BUYER BY ID (SECURE)");
//             log.info("========================================");
//             log.info("Buyer ID: {}", buyerId);
//             log.info("Org Admin ID: {}", orgAdminId);
            
//             // ✅ Verify Organization Admin exists
//             OrganizationAdmin admin = organizationAdminRepository.findById(orgAdminId)
//                     .orElseThrow(() -> new RuntimeException("Organization Admin not found"));
            
//             log.info("  [✓] Org Admin verified: {}", admin.getFullName());
//             log.info("  [✓] Company: {}", admin.getCompanyName());
            
//             // ✅ Fetch buyer ONLY if created by this admin
//             Buyer buyer = buyerRepository.findByIdAndCreatedByOrgAdminId(buyerId, orgAdminId)
//                     .orElseThrow(() -> new RuntimeException(
//                         "Buyer not found OR you don't have permission to view this buyer"
//                     ));
            
//             log.info("  [✓] Buyer found: {}", buyer.getCompanyName());
//             log.info("  [✓] Company Type: {}", buyer.getCompanyType());
//             log.info("  [✓] Organization: {}", buyer.getOrganizationCompanyName());
            
//             // Log structure
//             if (buyer.getLocations() != null) {
//                 log.info("  [✓] Locations: {}", buyer.getLocations().size());
//                 buyer.getLocations().forEach(location -> {
//                     log.info("     └─ {}", location.getLocationName());
//                     if (location.getDepartments() != null) {
...
//             log.info("========================================");
//             log.info("✅ BUYER RETRIEVED SUCCESSFULLY");
//             log.info("========================================");
...
//         } catch (Exception e) {
//             log.error("========================================");
//             log.error("❌ ERROR FETCHING BUYER");
//             log.error("========================================");
//             log.error("Buyer ID: {}", buyerId);
//             log.error("Admin ID: {}", orgAdminId);
//             log.error("Error: {}", e.getMessage());
//             e.printStackTrace();
//             log.error("========================================");
//             throw new RuntimeException("Failed to fetch buyer: " + e.getMessage(), e);
