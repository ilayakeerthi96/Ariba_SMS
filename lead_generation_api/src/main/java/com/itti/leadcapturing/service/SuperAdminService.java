...
public class SuperAdminService {
    ...
    private static final DateTimeFormatter DATE_FORMATTER =
            ...
            // ============================================================
// ADD THIS METHOD to SuperAdminService.java
// ============================================================

    @Transactional
    public ApiResponse<String> updateSuperAdminTheme(Long id, String theme) {
        try {
            log.info("🎨 Updating theme for SuperAdmin ID: {} → {}", id, theme);
            SuperAdmin superAdmin = superAdminRepository.findById(id)
                    .orElseThrow(() -> new RuntimeException("SuperAdmin not found"));
            superAdmin.setTheme(theme);
            superAdminRepository.save(superAdmin);
            log.info("✅ Theme updated: {}", theme);
            return ApiResponse.success("Theme updated successfully", theme);
        } catch (Exception e) {
            log.error("❌ Error updating theme for SuperAdmin: {}", id, e);
            return ApiResponse.error("Theme update failed", e.getMessage());
        }
    }

    public ApiResponse<String> getSuperAdminTheme(Long id) {
        try {
            SuperAdmin superAdmin = superAdminRepository.findById(id)
                    .orElseThrow(() -> new RuntimeException("SuperAdmin not found"));
            String theme = superAdmin.getTheme() != null ? superAdmin.getTheme() : "white";
    ...
    private SuperAdminResponse buildSuperAdminResponse(SuperAdmin superAdmin, String token) {
        ...
        return SuperAdminResponse.builder()
                ...
                .token(token)
                 .theme(superAdmin.getTheme() != null ? superAdmin.getTheme() : "default")
                .build();
    }
